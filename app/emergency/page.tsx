...
export default function EmergencyPage() {
  ...
  const [location, setLocation] = useState<string>("Waiting for GPS lock...");
    ...
    holdTimer.current = window.setTimeout(() => {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          () => setLocation("Location unavailable — enable GPS permissions")
        );
      } else {
  ...
  function shareLocation() {
    if (location.startsWith("Waiting") || location.includes("unavailable")) return;
    setShared(`Live location shared: ${location}`);
  }
    ...
    <main className="min-h-screen bg-[#05070d] pt-16">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
        <div>
          ...
          <button
            className={`mt-8 grid h-60 w-60 place-items-center rounded-full border text-center shadow-danger transition hover:scale-105 ${active ? "border-cyber-red bg-cyber-red/40 animate-pulse" : "border-cyber-red/50 bg-cyber-red/20"}`}
          ...
          >
            <span>
              ...
              <strong className="block text-3xl text-white">{active ? "ACTIVE" : "SOS"}</strong>
              <span className="mt-1 block text-sm text-white/70">{active ? "Emergency route engaged" : "Hold to activate"}</span>
            </span>
          </button>
          {active ? <p className="mt-4 text-sm text-cyber-mint">Live location: {location}</p> : null}
          {shared ? <p className="mt-2 text-sm text-white/70">{shared}</p> : null}
        </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-16 sm:px-6 md:grid-cols-3 lg:px-8">
        {emergencyActions.map((action, index) => (
          <Card key={action.title} className="p-5">
            ...
            <Button className="mt-5 w-full" variant="outline" onClick={index === 0 ? shareLocation : undefined}>
              Configure
            </Button>
          </Card>
        ))}
